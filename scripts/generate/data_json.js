...
const tokens = JSON.parse(fs.readFileSync(path.resolve('node_modules/@token-icons/core/dist/metadata/tokens.json')));
const networks = JSON.parse(fs.readFileSync(path.resolve('node_modules/@token-icons/core/dist/metadata/networks.json')));

// Define the path to the config file
const indexConfigPath = path.join(__dirname, '../../configs/index.config.json');
...
Object.keys(indexConf).forEach((mainnet, mainnetIndex) => {
  ...
  if (mainnetData.icon.id.indexOf('tokens') !== -1) {
    const iconMeta = tokens.find(token => token.id === mainnetData.icon.id.split('/')[1]);
    if (iconMeta) {
      mainnetData.icon.variants = iconMeta.variants;
    } else {
      warnings.push(`⚠️  Could not find icon meta for '${mainnetData.icon.id}'`);
    }
  }
  else {
    const iconMeta = networks.find(network => network.id === mainnetData.icon.id.split('/')[1]);
    if (iconMeta) {
      mainnetData.icon.variants = iconMeta.variants;
    } else {
      warnings.push(`⚠️  Could not find icon meta for '${mainnetData.icon.id}'`);
...
if (warnings.length) {
  ...
  console.log(`☑️  Data written to 'data.json' with ${warnings.length} warnings`);
} else {
